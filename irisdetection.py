...
class IrisDetector():
    def __init__(self):
        ...
        self.center_left, self.center_right, self.l_radius, self.r_radius = 0,0,0,0
    def Process(self,img): #returns the coordinates of the both iris and their radiuses
        ...
        if results.multi_face_landmarks:
            ...
            (l_cx, l_cy), self.l_radius = cv2.minEnclosingCircle(meshpoints[LEFT_IRIS])  #to get the centre of iris but n floating nums
            (r_cx,r_cy), self.r_radius = cv2.minEnclosingCircle(meshpoints[RIGHT_IRIS])

            self.center_left = np.array([l_cx,l_cy],dtype=np.int32) #origin coordinates of left iris in int
            self.center_right = np.array([r_cx, r_cy], dtype=np.int32) #origin coordinates of right iris in int
            return (self.center_left,self.center_right,self.l_radius,self.r_radius)
